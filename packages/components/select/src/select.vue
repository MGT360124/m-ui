<template>
  <div
    class="m-select"
    :class="[selectSize ? 'm-select--' + selectSize : '']"
    ...
    <div
      ...
      <span v-if="collapseTags && selected.length">
        <m-tag
          ...
          disable-transitions>
          <span class="m-select__tags-text">{{ selected[0].currentLabel }}</span>
        </m-tag>
        <m-tag
      ...
      <transition-group @after-leave="resetInputHeight" v-if="!collapseTags">
        <m-tag
          ...
          disable-transitions>
          <span class="m-select__tags-text">{{ item.currentLabel }}</span>
        </m-tag>
      </transition-group>
    ...
    <m-input
      ...
      <i slot="suffix"
       :class="['m-select__caret', 'm-input__icon', 'm-icon-' + iconClass]"
    ...
    <transition
      ...
      <m-select-menu
        ...
        <m-scrollbar
          tag="ul"
          wrap-class="m-select-dropdown__wrap"
          view-class="m-select-dropdown__list"
          ref="scrollbar"
        ...
        <p
          class="m-select-dropdown__empty"
          v-if="emptyText &&
...
<script type="text/babel">
  ...
  export default {
    ...
    computed: {
      ...
      emptyText() {
        if (this.loading) {
          return this.loadingText || this.t('m.select.loading');
        } else {
          if (this.remote && this.query === '' && this.options.length === 0) return false;
          if (this.filterable && this.query && this.options.length > 0 && this.filteredOptionsCount === 0) {
            return this.noMatchText || this.t('m.select.noMatch');
          }
          if (this.options.length === 0) {
            return this.noDataText || this.t('m.select.noData');
          }
        }
        return null;
      },
    ...
    watch: {
      ...
      visible(val) {
        if (!val) {
          this.handleIconHide();
          this.broadcast('MSelectDropdown', 'destroyPopper');
          if (this.$refs.input) {
          ...
          if (this.filterable) {
            ...
            if (this.multiple) {
              ...
              if (!this.remote) {
                this.broadcast('MOption', 'queryChange', '');
                this.broadcast('MOptionGroup', 'queryChange');
              }
              this.broadcast('MInput', 'inputSelect');
            }
          }
        }
    ...
    methods: {
      ...
      handleQueryChange(val) {
        ...
        this.previousQuery = val;
        this.$nextTick(() => {
          if (this.visible) this.broadcast('MSelectDropdown', 'updatePopper');
        });
        this.hoverIndex = -1;
        ...
        if (this.remote && typeof this.remoteMethod === 'function') {
          ...
          this.broadcast('MOption', 'queryChange', val);
          this.broadcast('MOptionGroup', 'queryChange');
        }
        if (this.defaultFirstOption && (this.filterable || this.remote) && this.filteredOptionsCount) {
          this.checkDefaultFirstOption();
